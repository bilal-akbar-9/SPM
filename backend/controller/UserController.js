...
const userController = {
	...
	getUserById: async (req, res) => {
		try {
			const user = await UserSchema.findById(req.params.id).select("-password").populate({
				path: "pharmacyId",
				localField: "pharmacyId",
				foreignField: "pharmacyId",
				model: "Pharmacy",
	...
	getMyData: async (req, res) => {
		try {
			const user = await UserSchema.findById(req.decoded.userId).select("-password").populate({
				path: "pharmacyId",
				localField: "pharmacyId",
				foreignField: "pharmacyId",
				model: "Pharmacy",
